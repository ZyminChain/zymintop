...
function getModePad(modelJson, index) {
    ...
    return getApiModelPad(modelJson.title, index, modelDom.innerHTML,modelJson.color||'success')
}

//获取一个Model外壳
function getApiModelPad(title, index, html, color) {
    let divDom = document.createElement('div')
    divDom.className = "panel panel-"+(color||'success')
    divDom.style = ""
    divDom.innerHTML = `
...
function tryApi(form) {
    ...
    if (form.getAttribute('method') == 'get') {
        ...
        $.ajax({
            ...
            res.status === 401 ? showSuccess(res) : showError(res.responseText)
        })
    }
    //如果是POST请求，表单提交
        ...
        $.ajax({
            ...
            contentType: false,
            headers: getHeader()
        }).done(function (res) { showSuccess(res) }).fail(function (res) { res.status === 401 ? showSuccess(res) : showError(res.responseText) })
    }
    //如果是PUT请求
        ...
        $.ajax({ type: "put", url: getUrl(form.getAttribute('action'), params), success: function (res) { showSuccess(res) }, headers: getHeader() }).fail(function (res) { res.status === 401 ? showSuccess(res) : showError(res.responseText) })
    }
    //如果请求是DELETE
        ...
        $.ajax({ type: "delete", url: getUrl(form.getAttribute('action'), params), success: function (res) { showSuccess(res) }, headers: getHeader() }).fail(function (res) { res.status === 401 ? showSuccess(res) : showError(res.responseText) })
    } else {
